...
for (iyear in 2020) {
  ...
  datestr = str_extract(string = lifiday, pattern = "[0-9]{13}")
  yrstr = as.numeric(substr(datestr, 1, 4))
  idx_yr = which(yrstr == iyear)
  lifiday = lifiday[idx_yr]
  datestr = datestr[idx_yr]
  ...
  {
    print(paste(iyear, ":", i,"of", nbday[i], sep = " "))
    lifim = lifiday[daystr==nbday[i]]
    nbim=length(lifim)
    if (nbim>0) {
